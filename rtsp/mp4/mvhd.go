...
func DefaultMvhdBox() *MvhdBox {
	return &MvhdBox{
		Version:          0,
		Flags:            [3]byte{0, 0, 0},
		CreationTime:     uint32(time.Now().Second()),
		ModificationTime: uint32(time.Now().Second()),
		Timescale:        1000,
		Duration:         ^0,
		Volume:           NewFixed16([]byte(uint16(0x100))),
		Rate:             NewFixed32([]byte(uint32(0x10000))),
	}
}
...
func DecodeMvhd(r io.Reader) (Box, error) {
	...
	return &MvhdBox{
		...
		Rate:             NewFixed32(data[20:24]),
		Volume:           NewFixed16(data[24:26]),
		notDecoded:       data[26:],
	}, nil
}
